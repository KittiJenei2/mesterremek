document.addEventListener('DOMContentLoaded', function() {
    // Megkeressük az összes jelszó-megjelenítő gombot
    ...
    toggleButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            e.preventDefault(); // Ne küldje el a formot véletlenül
            
            // Próbáljuk megkeresni az inputot a data-target alapján
            const targetId = this.getAttribute('data-target');
            let inputField = document.getElementById(targetId);
            
            // Ha el van írva a HTML-ben a data-target, B-terv: keressük meg a szülő elemen belül!
            if (!inputField) {
                inputField = this.parentElement.querySelector('input');
            }
            
            // Ha így sincs, kilépünk (ne fagyjon le a kód)
            if (!inputField) return; 
            
            // Váltás: password <-> text
